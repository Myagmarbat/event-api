...
app = FastAPI(title="Event API", description="API for managing events", version="0.1.0")
...
def create_event(event: schemas.EventCreate, db: Session = Depends(get_db)):
    ...
    if not event.event_type.strip():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="event_type cannot be empty"
        )
    if not event.user_id.strip():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="user_id cannot be empty"
        )
...
@app.delete(
    "/events/{event_id}", status_code=status.HTTP_204_NO_CONTENT, tags=["Events"]
)
